...
trait DecoderInstances:
  ...
  /** A decoder for `Int` values. Hint: use the `isValidInt` method of
    * `BigDecimal`.
    */
  given intDecoder: Decoder[Int] = Decoder.fromFunction {
    _ match
  ...
  given stringDecoder: Decoder[String] = Decoder.fromFunction {
    _ match
  ...
  given booleanDecoder: Decoder[Boolean] = Decoder.fromFunction {
    _ match
  ...
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction {
      _ match {
        case Json.Obj(m) => m.get(name).flatMap(decoder.decode(_))
        case _           => None
      }
    }
...
trait PersonCodecs:
  ...
  given Decoder[Person] =
    Decoder
      ...
      .transform(p => Person(name = p._1, age = p._2))
