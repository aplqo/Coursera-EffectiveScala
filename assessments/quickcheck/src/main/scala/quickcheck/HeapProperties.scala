  ...
  val meldingSmallHeaps: (String, Prop) =
    "melding a heap containing two low values with a heap containing two high values" ->
      forAll { (x: Int, y: Int) =>
        ...
        val (low, high) = if x > y then (y, x) else (x, y)
        def insertTwice(x: Int) = insert(x, insert(x, empty))

        val meldedHeap: List[Node] = meld(insertTwice(low), insertTwice(high))
  ...
  val meldingHeaps: (String, Prop) =
    "finding the minimum of melding any two heaps should return the minimum of one or the other of the source heaps" ->
      forAll { (heap1: List[Node], heap2: List[Node]) =>
        def check(melded: List[Node], h1: List[Node], h2: List[Node]): Boolean =
          if isEmpty(melded) then isEmpty(h1) && isEmpty(h2)
          else if !isEmpty(h1) && findMin(melded) == findMin(h1) then
            check(deleteMin(melded), deleteMin(h1), h2)
          else if !isEmpty(h2) && findMin(melded) == findMin(h2) then
            check(deleteMin(melded), h1, deleteMin(h2))
          else false
