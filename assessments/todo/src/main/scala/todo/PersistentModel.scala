...
object PersistentModel extends Model:
  ...
  private def loadAll() = (loadTasks(), loadId())
  private def saveAll(task: Tasks, id: Id) =
    saveTasks(task)
    saveId(id)

  def create(task: Task): Id =
    val (tasks, id) = loadAll()
    saveAll(Tasks(tasks.toMap + (id -> task)), id.next)
    id
  ...
  def update(id: Id)(f: Task => Task): Option[Task] =
    val tasks = loadTasks().toMap.updatedWith(id)(_.map(f))
  ...
  def complete(id: Id): Option[Task] =
    val tasks = loadTasks().toMap.updatedWith(id)(_.map(_.complete))
    saveTasks(Tasks(tasks))
    tasks.get(id)

  def tags: Tags =
    Tags(
      loadTasks().toMap.values
        .foldLeft(Set.empty[Tag])((res, task) => res ++ task.tags)
        .toList
    )

  def clear() = saveTasks(Tasks.empty)
