  ...
  def next: Firework =
    if this.countDown > 0 then
      this.copy(
        ...
        position = Motion
          .movePoint(this.position, this.direction, Settings.propulsionSpeed)
      )
    else
      Exploding.init(
        this.numberOfParticles,
...
case class Exploding(countDown: Int, particles: Particles) extends Firework:
  ...
  def next: Firework =
    if this.countDown > 0 then
      this.copy(countDown = this.countDown - 1, particles = this.particles.next)
    else Done
...
case class Particle(
  ...
  def next: Particle =
    ...
    val updatedHorizontalSpeed: Double = Motion.drag(this.horizontalSpeed)
    ...
    val updatedVerticalSpeed: Double =
      Motion.drag(this.verticalSpeed - Settings.gravity)
    ...
    this.copy(updatedHorizontalSpeed, updatedVerticalSpeed, updatedPosition)
